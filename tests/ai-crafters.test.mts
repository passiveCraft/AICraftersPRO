...
import {
  ...
  systemMetrics,
  unmatchedWorkflows,
} from '../lib/ai-crafters.ts';
import type { ExecutionDetail, Workflow } from '../lib/n8n-types.ts';
import { buildSystemWorkflow } from '../lib/system-workflows.ts';
import { layoutAgents } from '../lib/agent-layout.ts';

test('layout retains branch and disconnected coordinates without overlapping controls', () => {
  const value = workflow('w', 'Graph');
  value.nodes = Array.from({ length: 7 }, (_, i) => ({
    id: String(i),
    name: 'Agent ' + i,
    type: 'test',
    typeVersion: 1,
    disabled: i === 6,
    position: [i < 3 ? 0 : 300, i === 5 ? 400 : 0] as [number, number],
    parameters: {},
  }));
  value.edges = [
    { from: 'Agent 0', to: 'Agent 3', type: 'main' },
    { from: 'Agent 0', to: 'Agent 4', type: 'main' },
    { from: 'Agent 4', to: 'Agent 5', type: 'main' },
  ];
  const original = JSON.stringify(value);
  const layout = layoutAgents(value);
  ...
  for (let i = 0; i < points.length; i++)
    for (let j = i + 1; j < points.length; j++)
      assert.ok(
        Math.abs(points[i].x - points[j].x) >= 240 ||
          Math.abs(points[i].y - points[j].y) >= 170,
      );
  assert.deepEqual(layoutAgents(workflow('empty', 'Empty')).points, new Map());
});

test('unknown execution status is not counted as a completed run', () => {
  assert.equal(
    systemMetrics([
      {
        id: '1',
...
test('waiting, failed and crashed workflows do not imply activity in unreported Agents', () => {
  const value = workflow('w', 'Graph');
  const node = {
    id: 'a',
    name: 'Unreported',
  ...
  for (const status of ['waiting', 'running', 'error', 'crashed', 'success'])
    assert.equal(
      agentRunState(node, value, {
        id: 'e',
        workflowId: 'w',
        status,
        mode: '',
  ...
  assert.equal(
    agentRunState({ ...node, disabled: true }, value, null),
    'disabled',
  );
});
...
test('keeps archived and mapped workflows out of unmatched diagnostics', () => {
  const archived = { ...workflow('archived', 'Old automation'), archived: true };
  assert.deepEqual(
    unmatchedWorkflows([
      workflow('mapped', 'Product & Market Intelligence'),
      workflow('other', 'Unrelated Workflow'),
      archived,
    ]).map((item) => item.id),
    ['other'],
...
test('derives honest metrics from the loaded execution window', () => {
  const result = systemMetrics([
    ...
    },
    {
      ...
      mode: 'webhook',
      startedAt: '2026-01-01T00:00:00Z',
      stoppedAt: '2026-01-01T00:00:04Z',
    },
    {
      id: '3',
      workflowId: 'w',
...
test('derives Agent state from real node execution details and topology', () => {
  ...
  const detail: ExecutionDetail = {
    ...
    stoppedAt: null,
    lastNode: 'Start',
    steps: [{ name: 'Start', status: 'success', durationMs: 2 }],
  };
  assert.equal(agentRunState(value.nodes[0], value, detail), 'success');
  assert.equal(agentRunState(value.nodes[1], value, detail), 'unknown');
});
...
test('builds each ecommerce System with a runnable intake, Gemini Agent and explicit approval webhook', () => {
  ...
  assert.deepEqual(gemini?.credentials, {
    googlePalmApi: { id: 'cred-1', name: 'Gemini' },
  });
