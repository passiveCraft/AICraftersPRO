...
import { ArrowLeft, Bot, Box, Braces, Check, ChevronDown, ChevronRight, ChevronUp, CircleAlert, CirclePlay, Clock3, Code2, Copy, Download, FileUp, GitBranch, Globe2, Grip, LoaderCircle, MessageSquareText, MousePointer2, Play, Plus, Redo2, Save, Search, Sheet as SheetIcon, TerminalSquare, Trash2, Undo2, Webhook, X, ZoomIn, ZoomOut } from 'lucide-react';
...
const library: LibraryItem[] = [
  ...
  { label: 'Google Sheets', type: 'n8n-nodes-base.googleSheets', version: 4.5, icon: SheetIcon, description: 'Read or write a connected spreadsheet', parameters: { resource: 'sheet', operation: 'appendOrUpdateRow', documentId: { __rl: true, value: '', mode: 'list' }, sheetName: { __rl: true, value: '', mode: 'list' } } },
...
function NodeFields({ node, onChange }: { node: WorkflowNode; onChange: (parameters: Record<string, unknown>) => void }) {
  ...
  if (/googleSheets/i.test(node.type)) { const documentId = parameters.documentId && typeof parameters.documentId === 'object' ? parameters.documentId as Record<string, unknown> : {}; const sheetName = parameters.sheetName && typeof parameters.sheetName === 'object' ? parameters.sheetName as Record<string, unknown> : {}; const reference = (value: string, current: Record<string, unknown>) => ({ ...current, value }); return <div className="structured-fields"><span className="structured-title">GOOGLE SHEETS</span><p>Use a Google Sheets credential already configured in n8n. Secrets never appear here.</p><label><span>Action</span><select value={typeof parameters.operation === 'string' ? parameters.operation : 'appendOrUpdateRow'} onChange={event => update('operation', event.target.value)}><option value="appendOrUpdateRow">Append or update row</option><option value="getAll">Get rows</option><option value="append">Append row</option></select></label><label><span>Spreadsheet ID</span><Input placeholder="Google spreadsheet ID" value={typeof documentId.value === 'string' ? documentId.value : ''} onChange={event => update('documentId', reference(event.target.value, documentId))} /></label><label><span>Sheet tab</span><Input placeholder="Sheet1" value={typeof sheetName.value === 'string' ? sheetName.value : ''} onChange={event => update('sheetName', reference(event.target.value, sheetName))} /></label></div>; }
  if (/(gemini|openai|agent|chain)/i.test(node.type)) { const messages = parameters.messages && typeof parameters.messages === 'object' ? parameters.messages as Record<string, unknown> : {}; const values = Array.isArray(messages.values) ? messages.values : []; const first = values[0] && typeof values[0] === 'object' ? values[0] as Record<string, unknown> : {}; const prompt = typeof first.content === 'string' ? first.content : typeof parameters.prompt === 'string' ? parameters.prompt : ''; const setPrompt = (content: string) => values.length ? onChange({ ...parameters, messages: { ...messages, values: [{ ...first, content }, ...values.slice(1)] } }) : onChange({ ...parameters, prompt: content }); return <div className="structured-fields"><span className="structured-title">AGENT PROMPT</span><label><span>Instructions / message</span><Textarea className="prompt-field" spellCheck value={prompt} placeholder="Write the instructions this agent should follow…" onChange={event => setPrompt(event.target.value)} /></label><p>Expressions such as <code>{'{{ $json.customer }}'}</code> are preserved and resolved by n8n at run time.</p></div>; }
