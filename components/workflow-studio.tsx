...
import { ArrowLeft, Bot, Box, Braces, Check, ChevronDown, ChevronRight, ChevronUp, CirclePlay, Clock3, Code2, Copy, Download, FileUp, GitBranch, Globe2, Grip, LoaderCircle, MessageSquareText, MousePointer2, Play, Plus, Redo2, Save, Search, TerminalSquare, Trash2, Undo2, Webhook, X, ZoomIn, ZoomOut } from 'lucide-react';
...
export function WorkflowStudio({ workflow: initial, instanceUrl, onClose, onChanged }: Props) {
  ...
  async function runWorkflow() {
    ...
    if (!workflow.id || dirty) {
      const message = dirty ? 'save changes before running this workflow' : 'save this workflow before running it';
      setError(dirty ? 'Save your changes before running this workflow.' : 'Save this workflow before running it.');
      setRunLog(items => [`${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })} · blocked · ${message}`, ...items].slice(0, 8));
      return;
    }
    setBusy('run'); setError('');
    setRunLog(items => [`${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })} · run started · ${workflow.name}`, ...items].slice(0, 8));
    try {
      ...
      setMessage('Workflow execution completed'); await onChanged();
    }
    catch (issue) {
  ...
  return <main className={`studio-shell ${consoleOpen ? 'console-open' : 'console-closed'}`}>
    ...
    <button className="studio-execute-float" onClick={() => hasWebhookTrigger ? void runWorkflow() : instanceUrl && window.open(`${instanceUrl}/workflow/${encodeURIComponent(workflow.id)}`, '_blank', 'noopener,noreferrer')} disabled={!!busy || !workflow.id || (hasWebhookTrigger && (!workflow.active || dirty))} title={hasWebhookTrigger ? 'Send the JSON test input to the active webhook' : hasManualTrigger ? 'Manual execution uses n8n session access' : 'Open this workflow in n8n'}><Play size={17} /><strong>{busy === 'run' ? 'Running workflow' : hasWebhookTrigger ? 'Execute workflow' : 'Open to run'}</strong></button>
