...
import { ArrowLeft, Bot, Box, Braces, Check, ChevronRight, CirclePlay, Clock3, Code2, Copy, Download, FileUp, GitBranch, Globe2, Grip, LoaderCircle, MessageSquareText, MousePointer2, Play, Plus, Redo2, Save, Search, Trash2, Undo2, Webhook, X, ZoomIn, ZoomOut } from 'lucide-react';
...
export function WorkflowStudio({ workflow: initial, instanceUrl, onClose, onChanged }: Props) {
  ...
  async function runWorkflow() {
    if (!workflow.id || dirty) { setError(dirty ? 'Save your changes before running this workflow.' : 'Save this workflow before running it.'); return; }
    setBusy('run'); setError('');
    try { const input = JSON.parse(testInput); await api<{ status: string; output: unknown }>(`?operation=trigger&workflowId=${encodeURIComponent(workflow.id)}`, { method: 'POST', body: JSON.stringify(input) }); setMessage('Webhook test completed'); await onChanged(); }
    catch (issue) { setError(issue instanceof SyntaxError ? 'Test input must be valid JSON.' : issue instanceof Error ? issue.message : 'Could not trigger the workflow.'); }
    finally { setBusy(''); }
  }
